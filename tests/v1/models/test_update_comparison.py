...
def test_default_dict_lookup_by_instance():
    ...
    put = features.OnFeature(on=True, func_class="toggle", func_instance="zone-1")
...
def test_default_dict_lookup_by_class_instance_tuple():
    ...
    key = ("toggle", "zone-1")
    cached = features.OnFeature(on=True, func_class="toggle", func_instance="zone-1")
...
def test_features_equivalent_identity_and_none():
    feat = features.OnFeature(on=True, func_class="power", func_instance=None)
...
def test_features_equivalent_ignores_dimming_supported():
    ...
    cached = features.DimmingFeature(
        brightness=40, supported=[0, 100], func_instance="color"
    )
    assert features_equivalent_for_update(put, cached)


def test_features_equivalent_open_and_fallback():
    open_a = features.OpenFeature(open=True, func_class="toggle", func_instance="s1")
    open_b = features.OpenFeature(open=True, func_class="toggle", func_instance="s1")
    open_c = features.OpenFeature(open=False, func_class="toggle", func_instance="s1")
        ...
        (),
        {
            "on": {
                None: features.OnFeature(
                    on=False, func_class="power", func_instance=None
                )
            }
        },
    )()
    put = features.OnFeature(on=False, func_class="power", func_instance=None)
...
def test_light_protocol_and_channel_toggle_gate():
    ...
    toggle = features.OnFeature(on=True, func_class="toggle", func_instance="color")
    ...
    power = features.OnFeature(on=True, func_class="power", func_instance=None)
