...
class SecuritySystemSensorController(BaseResourcesController[SecuritySystemSensor]):
    ...
    async def set_state(
        ...
        update_obj = SecuritySystemSensorPut(callback=self.update_from_states)
        try:
    ...
    async def update_from_states(
        self, cur_item: SecuritySystemSensor, update_vals: SecuritySystemSensorPut
    ) -> set[str]:
        """Update the item from the incoming raw states."""
        modified_states: list[AferoState] = get_valid_states(
            [AferoState(**update_vals.sensor_config.api_value)], cur_item.instance
        )
        return await self._update_elem(cur_item, modified_states)
