...
Afero accounts use OpenID Connect with refresh tokens. aioafero handles the
...
There are two ways to construct ``AferoAuth`` (both require ``session``):
...
Pass a saved ``refresh_token`` (and optionally a still-valid bearer ``token``) via
``AferoAuth(session, username, refresh_token, token=..., token_expiration=...)`` or
``AferoBridgeV1``. When passing
``token``, also pass ``token_expiration`` (Unix timestamp) if you know when it expires;
otherwise the library treats the bearer as stale and refreshes on first use. OAuth
responses still use ``expires_in`` minus a 2-second buffer.
...
Both ``AferoAuth`` and ``AferoBridgeV1`` require an ``aiohttp.ClientSession`` at
construction. Create one (as above) and pass it to login and the bridge. The bridge does
**not** close a session you pass in — call ``await session.close()`` after
``await bridge.close()``.

Alternatively, :meth:`~aioafero.v1.AferoBridgeV1.open` may create a session when
``session`` is omitted; that owned session is closed by ``await bridge.close()``.
...
password, Afero **emails** a one-time code to the account, and
...
``session``, and optional ``token``. You do not need a separate auth object for normal
bridge use after the initial login.

Typical integration pattern (e.g. Home Assistant):

1. **Setup** — ``for_login`` → ``login()`` / ``submit_otp()`` → save ``refresh_token``
   in config storage.
2. **Runtime** — ``AferoBridgeV1(username, refresh_token, session)`` on every start; never persist
   the password.
