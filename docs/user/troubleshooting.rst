...
so the device maps to the correct ``device_class``. See :doc:`../contributing`.
...
* ``OTPRequired`` / ``InvalidOTP`` — collect the emailed code from the user and call
  ``auth.submit_otp`` on the same ``AferoAuth`` instance (:doc:`auth`).
