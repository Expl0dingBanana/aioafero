...
* **Bridge** — ``AferoBridgeV1`` owns controllers, polling, and the HTTP session (or
  shares one you provide).
...
2. Create ``AferoBridgeV1`` with ``username`` and ``refresh_token``.
...
8. ``await bridge.close()`` when finished.
