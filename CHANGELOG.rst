 ...
 * ``AferoBridgeV1`` and ``AferoAuth`` require an ``aiohttp.ClientSession`` at
   construction (``AferoBridgeV1.open`` may create one when omitted).
 ...
 * ``AferoAuth`` is decoupled from the bridge and uses ``aiohttp.ClientSession``
   ...
   login; ``login()`` and ``submit_otp()`` return ``TokenData``. Use
   ``AferoAuth(session, username, refresh_token, token=..., token_expiration=...)``
   for runtime auth.
 ...
 * Dependency floors: ``aiohttp>=3.14.3``, ``beautifulsoup4>=4.12.0`` (``aiohttp``
   3.14.3 addresses published CVEs).
